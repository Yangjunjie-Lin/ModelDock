...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "The V21 through V25 application migration failed." }
    ...
    if ($upgrade -ne "25|1|1|1|4") { throw "The V21 fixture was not upgraded through V25, upgraded additively, or the existing supplier was not safely disabled." }
    Write-Host "PASS populated V21 database upgraded through V25 with the existing supplier and disabled payout policy preserved"
