...
if ($LASTEXITCODE -ne 0 -or @($inspect).Count -ne 1 -or -not [bool]$inspect[0].State.Running -or
    [string]$inspect[0].Config.Labels.'com.docker.compose.service' -ne "postgres") { throw "The expected local RelayDock PostgreSQL service is not running." }
...
$databaseName = "relaydock_subscription_test_" + [Guid]::NewGuid().ToString("N").Substring(0, 20)
$created = $false
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Creating the disposable subscription database failed." }
    $created = $true
    ...
    docker run --rm --network $network --env TEST_DATABASE_URL -v relaydock-go-mod-cache:/go/pkg/mod -v "${repoRoot}:/src" -w /src golang:1.26.6-alpine go test -count=1 -run TestSubscriptionsIntegration ./internal/store
    if ($LASTEXITCODE -ne 0) { throw "Subscription integration test failed." }
    Write-Host "PASS subscription concurrency, immutable versions/invoices, lifecycle, data retention, and Token/subscription ledger separation"
} finally {
    if ($created -and $databaseName -match '^relaydock_subscription_test_[0-9a-f]{20}$') {
