...
$workflowAttempt = if ($env:GITHUB_RUN_ATTEMPT) { $env:GITHUB_RUN_ATTEMPT } else { "LOCAL" }
...
function Invoke-Suite([string]$Name, [scriptblock]$Action) {
    ...
    try {
        ...
        $script:suiteResults.Add([ordered]@{ id=$Name; status="PASS" })
    } catch {
        $script:suiteResults.Add([ordered]@{ id=$Name; status="FAIL" })
...
$level = if ($env:GITHUB_ACTIONS -ceq "true") { "ENGINEERING_CI" } else { "DEVELOPMENT" }
$origin = if ($env:GITHUB_ACTIONS -ceq "true") { "GITHUB_ACTIONS_ARTIFACT" } else { "LOCAL_DEVELOPMENT" }

if ($CandidateMetadataDirectory) {
    if ($env:GITHUB_ACTIONS -cne "true" -or $workflowRun -eq "LOCAL" -or $workflowAttempt -eq "LOCAL") {
        throw "RELEASE_CANDIDATE evidence can only be generated by GitHub Actions."
    }
    ...
    foreach ($component in @("server", "admin", "console")) {
        ...
        if ([string]$item.component -cne $component -or [string]$item.commit -cne $commit -or
            [string]$item.digest -cne [string]$digests[$component] -or [string]$item.repository -cne $repository -or
            [string]$item.workflow_run_id -cne [string]$workflowRun -or [string]$item.workflow_run_attempt -cne [string]$workflowAttempt) {
            throw "Candidate metadata identity mismatch for $component."
        }
        if ([string]$item.evidence_level -notin @("ENGINEERING_CI", "RELEASE_CANDIDATE")) {
            throw "Candidate metadata evidence level is invalid for $component."
        }
        if ($metadataLevel -and $metadataLevel -cne [string]$item.evidence_level) {
            throw "Candidate metadata evidence levels do not match."
        }
        $metadataLevel = [string]$item.evidence_level
...
try {
    ...
    $evidence = [ordered]@{
        ...
        commit_sha = $commit
        tree_sha = $tree
        branch_or_tag = "$refType/$refName"
        migration_version = $migration
        version = $version
        workflow_run_id = [string]$workflowRun
        workflow_run_attempt = [string]$workflowAttempt
        server_image_digest = $digests.server
        admin_image_digest = $digests.admin
        console_image_digest = $digests.console
...
Write-Host "PASS commercial integration suites=$($suiteResults.Count) commit=$commit tree=$tree migration=$migration server=$($digests.server)"
