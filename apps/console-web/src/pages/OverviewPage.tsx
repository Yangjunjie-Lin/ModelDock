...
import { api, decimalRatioPercent, formatMoney, formatNumber } from '../lib/api'
...
export function OverviewPage() {
  ...
  return <div className="page-stack">
    <div className="page-header console-welcome"><div><div className="eyebrow-row"><span className="live-dot" />GATEWAY READY</div><h1>Build with one endpoint.</h1><p>Use stable model aliases, RelayDock API keys, and OpenAI-compatible SDKs.</p></div><div className="header-actions"><Link to="/console/docs"><Button>Read the docs</Button></Link><Link to="/console/api-keys"><Button variant="primary"><KeyRound size={15} />Create API key</Button></Link></div></div>
    ...
    {data && <>
      ...
      <div className="console-overview-grid">
        ...
        <Panel title="Monthly limits" description="Workspace consumption for the current cycle"><Quota label="Tokens" value={String(data.monthly_tokens_used || 0)} limit={String(data.monthly_token_limit || 0)} formatter={formatNumber} /><Quota label="Estimated cost" value={data.monthly_cost_used || '0'} limit={data.monthly_cost_limit || '0'} formatter={formatMoney} /><div className="quota-footer"><span><KeyRound size={14} />{`${formatNumber(data.active_api_keys, false)} active keys`}</span><span><Layers3 size={14} />{`${formatNumber(data.available_models, false)} models`}</span></div></Panel>
        <Panel className="quickstart-panel" title="Quick start" description="OpenAI Python SDK" action={<Button size="sm" onClick={() => { void navigator.clipboard.writeText(code); toast('Code copied') }}><Copy size={13} />Copy</Button>}><pre className="code-block"><code>{code}</code></pre><Link className="run-playground-link" to="/console/playground"><Braces size={14} />Try it in the Playground<ArrowRight size={13} /></Link></Panel>
        <Panel title="Recent requests" description="Sanitized request metadata" action={<Link className="text-link" to="/console/logs">View all</Link>}>{data.recent_requests?.length ? <div className="recent-list">{data.recent_requests.map((row, index) => <div key={String(row.request_id || index)}><div><code>{String(row.request_id || '—')}</code><span>{String(row.requested_model || row.model || '—')}</span></div><div><StatusBadge value={row.status_code} /><span>{formatNumber(row.total_tokens)} tok</span><span>{formatNumber(row.latency_ms, false)} ms</span></div></div>)}</div> : <EmptyState title="No recent requests" />}</Panel>
      </div>
    </>}
  </div>
}
...
function normalizeOverview(value: Overview & Record<string, unknown>): Overview {
  ...
  return {
    ...
    estimated_cost_today: value.estimated_cost_today ?? String(value.estimated_cost ?? '0'),
...
function Quota({ label, value, limit, formatter }: { label: string; value: string; limit: string; formatter: (value: unknown) => string }) {
  const percent = decimalRatioPercent(value, limit)
  const hasLimit = percent !== null
  return <div className="quota-row"><div><strong>{label}</strong><span>{`${formatter(value)} / ${hasLimit ? formatter(limit) : 'No limit'}`}</span></div><div className="quota-track"><i style={{ width: `${percent ?? 0}%` }} /></div><small>{hasLimit ? `${(percent ?? 0).toFixed(1)}% used` : 'No workspace limit configured'}</small></div>
}
