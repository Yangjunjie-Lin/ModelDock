...
function BudgetMeter({ label, value, limit, format }: { label: string; value: number; limit: number; format: (value: unknown) => string }) {
  const percent = limit > 0 ? Math.min(100, value / limit * 100) : 0
  return <div><span>{label}</span><strong>{format(value)} <small>/ {limit > 0 ? format(limit) : 'No limit'}</small></strong><div className="quota-track"><i style={{ width: `${percent}%` }} /></div><small>{limit > 0 ? `${percent.toFixed(1)}% used` : 'No active limit'}</small></div>
}
